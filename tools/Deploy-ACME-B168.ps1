...
try {
    ...
    if (-not $startup.Contains('ACME_buildBatch = "B168";')) { throw 'This checkout is not B168.' }
    ...
    Write-Host "B168 deployed to $target. Restart Arma/server and check the B168 debug marker."
} finally {
    Pop-Location
}
