...
try {
    ...
    if (-not $startup.Contains('ACME_buildBatch = "B162";')) { throw 'This checkout is not B162.' }
    ...
    Write-Host "B162 deployed to $target. Restart Arma/server and check the B162 debug marker."
} finally {
    Pop-Location
}
