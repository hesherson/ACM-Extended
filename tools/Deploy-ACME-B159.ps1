...
try {
    ...
    if (-not $startup.Contains('ACME_buildBatch = "B159";')) { throw 'This checkout is not B159.' }
    ...
    Write-Host "B159 deployed to $target. Restart Arma/server and check the B159 debug marker."
} finally {
    Pop-Location
}
