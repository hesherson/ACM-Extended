...
try {
    ...
    if (-not $startup.Contains('ACME_buildBatch = "B160";')) { throw 'This checkout is not B160.' }
    ...
    Write-Host "B160 deployed to $target. Restart Arma/server and check the B160 debug marker."
} finally {
    Pop-Location
}
