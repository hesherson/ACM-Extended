...
try {
    ...
    if (-not $startup.Contains('ACME_buildBatch = "B166";')) { throw 'This checkout is not B166.' }
    ...
    Write-Host "B166 deployed to $target. Restart Arma/server and check the B166 debug marker."
} finally {
    Pop-Location
}
