...
check('retired provider DraggerBase wrapper absent', 'class ACME_HeadElevProviderLift: DraggerBase' not in cfg)
...
check('provider waits for one preflight',
      ...
      and '_prepUntil' in seq
      and 'currentWeapon _u != "" && {_now < _prepUntil}' in seq)
check('duplicate holster suppression',
      '_elapsed >= 0 && {_elapsed < 3.2}' in prep
      and 'ace_weaponselect_fnc_putWeaponAway' in prep)
