...
def hover_source(text=None):
    ...
    text = text.replace('_colors getOrDefault [_gauge,[1,1,1,0.56]]', '[_colors,[_gauge,[1,1,1,0.56]]] call _getDefault')
...
def test_hover_stock_count_badge_uses_live_tile_and_collapses_on_exit(gauge, stock):
    execute(setup(gauge, stock) + f'''
        ...
        [count _fan==5 && {{count _created==5}},"fan creation lost a control"] call _check;
        ...
        [count _created==5,"exit recreated fan controls"] call _check;
