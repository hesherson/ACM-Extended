...
def test_hang_bag_normal_entry_and_exit_blend():
    ...
    ack = text('functions/fn_hangBagClaimAck.sqf')
    ...
    # Presentation begins only after the patient owner accepts the exact lease.
    assert '"hangBagClaim"' in start
    assert 'ACME_fnc_hangBagActivate' in ack
    assert '[_medic, _inAnim, 1.4, 1] call ACME_fnc_doAnimHeld;' in activate
    assert 'Renewals/duplicate ACKs never repeat props' in ack
