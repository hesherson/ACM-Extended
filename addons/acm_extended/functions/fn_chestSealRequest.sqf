/* ACE shared-supply reservation plus an idempotent, field-level intention. Pending requests
...
private _receipt = [];
if (_item != "") then {
    _receipt = [_medic, _patient, [_item]] call ACME_fnc_treatmentSupplyTake;
};
if (_item != "" && {_receipt isEqualTo []}) exitWith {false};
...
ACME_CS_pending set [_id, [_request, _receipt, diag_tickTime]];
