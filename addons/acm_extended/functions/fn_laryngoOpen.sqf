...
if (([_medic, _patient, "ACME_Laryngoscope"] call ACME_fnc_treatmentSupplyCount) < 1) exitWith {
...
if (!(uiNamespace getVariable ["ACME_laryngo_resume", false])
    && {([_medic, _patient, "ACME_ETTube"] call ACME_fnc_treatmentSupplyCount) < 1}) exitWith {
