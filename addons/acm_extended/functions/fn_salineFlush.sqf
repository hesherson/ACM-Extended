...
switch (_mode) do {
    ...
    case "prep": {
        if (!isNil "ACME_fnc_itemCount" && {([_medic, "ACM_SalineFlush_10"] call ACME_fnc_itemCount) < 1}) exitWith {
            ["No 10 mL saline flush in inventory.", 2, _medic] call ace_common_fnc_displayTextStructured;
        };
        [_medic, "ACM_SalineFlush_10"] call ACME_fnc_itemTake;
    ...
    case "flushLine": {
        ...
        if (([_medic, "ACM_SalineFlush_10"] call ACME_fnc_itemCount) < 1) exitWith {};
