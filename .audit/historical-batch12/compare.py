...
def compare(folder):
    ...
    summary={'fixed_count':len(fixed),'fixed':sorted(k[0] for k in fixed),'regressions':regress,'missing':missing,'new_failures':newfails,'errors':errors,'skips':{l:sum(r['outcome']=='skipped' for r in rs) for l,rs in reports.items()},'broad_remains_failing':True}
    return summary
if __name__=='__main__':
    summary=compare(W/'results');(W/'results/comparison.json').write_text(json.dumps(summary,indent=2));print(json.dumps(summary,indent=2))
